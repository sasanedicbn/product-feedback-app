...
const DropDown = ({ options, selectedOption, onOptionSelect }) => {
    ...
    return (
        <div>
            ...
            <ul
                className={`absolute bg-white w-[200px] h-[150px] mt-9 rounded-lg shadow-xl border border-solid border-gray-200 transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}
            >
                {options.map((option) => (
                    <li
                        key={option}
